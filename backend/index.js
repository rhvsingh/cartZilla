        ...
        if (err) {
            ...
            transporter.sendMail(mailOptions, function (error, info) {
                if (error) {
                    ...
                    res.json({ 'email': email, 'name': name, 'register': true, 'otpStatus': true, 'msg': 'Register successful' });
                }
            });
...
app.post('/addProduct', (req, res) => {
    ...
    let productData = { 'pid': pid, 'img': img, 'name': name, 'desc': desc, 'price': price, 'discount': discount };
...
app.post('/addCart', (req, res) => {
    ...
    if (qty == -1 || qty == 1) {
        productsCollection.find({ 'pid': pid }).limit(1).sort({ _id: -1 }).toArray((err, result) => {
            if (err) {
                console.log(err.message);
            } else if (result.length) {
                let data = { 'pid': pid, 'qty': qty, 'cId': cId };

                cartCollection.find({ 'pid': pid }).limit(1).sort({ _id: -1 }).toArray((err2, result2) => {
                    if (err2) {
                        console.log(err2.message)
                    } else if (result2.length) {
                        let newQty = result2[0].qty + qty;
                        if (newQty > 0) {
                            cartCollection.updateOne({ 'pid': pid }, { $set: { 'qty': newQty } }, (err3, result3) => {
                                if (err3) {
                                    console.log(err.message);
                                } else {
                        ...
                        if (qty <= 0) {
                            res.json({ 'result': false, 'msg': 'Invalid quantity number. Cart cannot be updated.' });
                        } else {
        ...
        res.json({ 'result': false, 'msg': 'Invalid quantity number. Alteration in Cart not possible' });
    }
})
...
app.post('/addCart/:akey', (req, res) => {
    ...
    if (qty == -1 || qty == 1) {
        userCollection.find({ 'akey': akey }).limit(1).sort({ _id: -1 }).toArray((err4, result4) => {
            if (err4) {
                console.log(err4)
                ...
                productsCollection.find({ 'pid': pid }).limit(1).sort({ _id: -1 }).toArray((err, result) => {
                    if (err) {
                        ...
                        cartCollection.find({ 'pid': pid, 'email': email }).limit(1).sort({ _id: -1 }).toArray((err2, result2) => {
                            if (err2) {
                                console.log(err2.message)
                            } else if (result2.length) {
                                let newQty = result2[0].qty + qty;
                                if (newQty > 0) {
                                    ...
                                    cartCollection.updateOne({ 'pid': pid, 'email': email }, { $set: { 'qty': newQty, 'tprice': tprice } }, (err3, result3) => {
                                        if (err3) {
                                            console.log(err.message);
                                        } else {
                                            res.json({ 'result': true, 'msg': 'Cart Updated' });
...
app.get('/showCart/:akey', (req, res) => {
    ...
    userCollection.find({ 'akey': akey }).limit(1).toArray((err, result) => {
        if (err) {
            console.log(err)
            res.json({ 'err': err })
        } else if (result.length) {
            ...
            cartCollection.find({ 'email': email }).toArray((err2, result2) => {
                if (err2) {
                    ...
                    for (let i = 0; i < result2.length; i++) {
                        ...
                        cartTotalCalcPrice = cartTotalCalcPrice + parseInt(result2[i].tprice)
                    }
                    res.json({ 'result': true, 'count': result2.length, 'totalQty': cartTotalQty, 'tCalcPrice': cartTotalCalcPrice })
                } else {
                    res.json({ 'result': false, 'count': null, 'totalQty': null })
                }
            })
...
app.post('/product/img/test', upload.single('productImage'), (req, res, next) => {
    if (!req.file) {
        res.send({ code: 500, msg: 'err' })
    } else {
        //console.log(req.file)
        res.send({ code: 200, imgName: req.file.filename, msg: 'Upload successful' })
    }
})
