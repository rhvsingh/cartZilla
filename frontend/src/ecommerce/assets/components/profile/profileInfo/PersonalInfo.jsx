...
const PersonalInfo = ({ userDetails }) => {
    ...
    function RadioButtonsShow({disabled,gender}) {
        return (
            <div className={'my-1 ' + ProfileStyle.radioButtonContainer} onChange={onChangeRadio}>
                <label htmlFor="male_radio_button">
                    <input type="radio" name='gender' value="male" id='male_radio_button' defaultChecked={gender === 'male' && true} disabled={disabled} />
                    Male
                </label>
                <label htmlFor="female_radio_button">
                    <input type="radio" name='gender' value="female" id='female_radio_button' defaultChecked={gender === 'female' && true} disabled={disabled} />
                    Female
                </label>
            </div>
        )
    ...
    return (
        <div className={ProfileStyle.marginBottom}>
            ...
            <div>
                Your Gender
                <br />
                {nameGen.gender === '' ? <RadioButtonsShow disabled={disabled} /> : <RadioButtonsShow disabled={disabled} gender={nameGen.gender} />}
