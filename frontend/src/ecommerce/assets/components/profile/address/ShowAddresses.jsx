...
const ShowEachAddress = lazy(() => import("./ShowEachAddress"))
...
const ShowAddresses = ({ userDetails, setUserDetails }) => {
    ...
    const deleteAddress = async (address_id) => {
        const confirmBox = window.confirm("Do you really want to delete this address?")

        if (confirmBox) {
            let newArray = userDetails.addresses.filter((item) => item.address_id !== address_id)
            ...
            await axios.post(baseURL + "user/address/delete", data).then((response) => {
    ...
    return (
        <div className="address-container">
            {userAddresses && userAddresses.length ? <ShowAddressesScreen /> : "No Addresses"}
        </div>
    )
}
